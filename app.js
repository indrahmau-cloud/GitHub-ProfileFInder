...
async function fetchProfile() {
  ...
  try {
    // No token needed for public profiles (60 req/hour limit applies)
    // To increase to 5,000/hr: create a token at https://github.com/settings/tokens
    // Then replace the empty string below with your token:
    const TOKEN = '';
...
function render(u, repos) {
  ...
  if (u.location) {
    chips.push(`<span class="chip"><svg viewBox="0 0 16 16"><path d="M8 0a5 5 0 00-5 5c0 3.75 5 11 5 11s5-7.25 5-11a5 5 0 00-5-5zm0 7a2 2 0 110-4 2 2 0 010 4z"/></svg>${esc(u.location)}</span>`);
  }

  if (u.company) {
    chips.push(`<span class="chip"><svg viewBox="0 0 16 16"><path d="M1.5 14.5v-13h9v13h-9zm1-12v11h7v-11h-7zm6 2h-5v1h5v-1zm0 2h-5v1h5v-1zm0 2h-5v1h5v-1zm5 8v-8l-3-3v11h3z"/></svg>${esc(u.company.replace(/^@/, ''))}</span>`);
  }
...
function buildRepoCard(r) {
  ...
  const langHtml = r.language
    ? `<span class="repo-badge"><span class="lang-dot" style="background:${dotColor}"></span>${esc(r.language)}</span>` : '';
